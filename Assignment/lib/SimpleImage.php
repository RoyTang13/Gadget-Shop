...
// Resolve directory path (support relative 'photos' from project root)
        $dir = dirname($file);
        if (!preg_match('/^(?:[A-Za-z]:|[\\\\\/])/', $dir)) {
            $base = realpath(__DIR__ . '/..') ?: (__DIR__ . '/..');
            $dir = rtrim($base, '/\\') . DIRECTORY_SEPARATOR . $dir;
        }

        // Ensure target directory exists
        if (!is_dir($dir)) {
            if (!mkdir($dir, 0755, true) && !is_dir($dir)) {
                throw new Exception("Failed to create directory: $dir", self::ERR_WRITE);
            }
        }

        // Attempt to write the file (report OS error if it fails)
        $target = rtrim($dir, '/\\') . DIRECTORY_SEPARATOR . basename($file);
        if (false === @file_put_contents($target, $image['data'])) {
            $err = error_get_last();
            $msg = isset($err['message']) ? $err['message'] : 'unknown';
            throw new Exception("Failed to write image to file: $target. OS error: $msg", self::ERR_WRITE);
        }
